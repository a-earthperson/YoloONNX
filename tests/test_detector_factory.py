...
class TestDetectorFactory(unittest.TestCase):
    def test_resolve_backend_from_model_suffix(self):
        self.assertEqual(resolve_backend(make_config(model_file="model.tflite")), "tflite")
    ...
    def test_tflite_backend_requires_label_file(self):
        with self.assertRaises(ValueError):
            create_detector(make_config(backend="tflite", model_file="model.tflite", label_file=None))

    def test_onnx_backend_is_constructed_with_optional_labels(self):
        config = make_config(backend="onnx", model_file="model.onnx", label_file=None, device="cuda:1")
        with patch("detector_factory.ONNXDetector") as detector_cls:

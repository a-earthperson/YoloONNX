...
FROM ${PYTHON_IMAGE} AS yolo-frigate-onnx-builder
...
RUN --mount=type=cache,target=/root/.cache/uv,sharing=locked \
    uv sync --frozen --no-dev --no-editable --extra onnx

FROM ${PYTHON_IMAGE} AS yolo-frigate-onnx
...
COPY --from=yolo-frigate-onnx-builder /app/.venv /app/.venv
